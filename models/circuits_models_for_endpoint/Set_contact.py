...
def set_full_contact(engine,Form:Contact_Model) -> Result_model_function:
    with Session(engine) as session:
        try:
            session.add(Contact(name=Form.name,subject=Form.subject,body=Form.body,number=Form.number,begining=Form.begining,number_of_person=Form.number_of_person,mail=Form.mail))
            session.commit()  
