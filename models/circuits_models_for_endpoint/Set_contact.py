...
def set_full_contact(engine,Form:Contact_Model) -> Result_model_function:
    with Session(engine) as session:
        try:
            session.add(Contact(name=Form.name,subject=Form.subject,body=Form.body,circuit_id=Form.circuit_id,number=Form.number,begining=Form.begining,number_of_person=Form.number_of_person,mail=Form.mail,total_price=Form.total_price))
            session.commit()  
